...
## Назначение
Папка `models` содержит общие Data Transfer Objects (DTO) - модели данных, которые используются несколькими фичами приложения. Эти классы представляют структуру данных для API, локальной БД и других внешних систем.
...
- shared_model.dart - 🎯 Базовые DTO классы и интерфейсы

## Описание

### model.dart
Базовые модели данных с использованием json_annotation. Содержит:
- Аннотированные классы для всех общих моделей
- Базовые DTO классы с кодогенерацией
...
### BaseModel
Абстрактный класс для всех общих моделей с json_annotation:
- Базовые поля (id, createdAt, updatedAt) с аннотациями
- Автогенерируемые методы сериализации/десериализации
- Стандартная валидация общих данных
- Автоматическое сравнение моделей

### SharedDataTransferObject
Интерфейс для общих DTO классов с кодогенерацией:
...
- Стандартные методы для общих данных

## Преимущества

- **Типобезопасность** - строгая проверка типов при сериализации
- **Автоматизация** - кодогенерация исключает ручные ошибки
- **Стандартизация** - единая структура для всех общих моделей
- **Согласованность** - одинаковые подходы к сериализации между фичами
...
// Базовый класс общей модели с json_annotation
@JsonSerializable()
...
// Общая модель пользователя для всех фич
@JsonSerializable()
